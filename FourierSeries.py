...
class FourierSeries:
    ...
    def plot(self, ax, wave_type="square"):
        ...
        original = target_function(x, wave_type)
...
def target_function(x, function_type="square"):
    ...
    if function_type == "square":
        ...
        y = np.zeros_like(x)
        period = 2 * np.pi
        y = (x + np.pi) % period - np.pi
        return y

    elif function_type == "triangle":
        # Mathematical Triangle Wave (Odd Function)
        y = (x + np.pi) % (2*np.pi) - np.pi
        return np.where(abs(y)<=np.pi/2, (2/np.pi)*y, np.sign(y)*2 - (2/np.pi)*y)

    elif function_type == "cubic":
        # Periodic Cubic: x^3 defined on -1 to 1, repeated.
        y = np.zeros_like(x)
        period = 2  # period is 2 from -1 to 1
        y = (x + 1) % period - 1  # map x to -1 theke 1 range
        return y**3
        ...
        y = np.zeros_like(x)
        period = 2 * np.pi
        y[np.abs((x + period/2) % period - period/2) < 0.1] = 1
        return y
...
def get_half_period(wave_type):
    ...
    """
    if wave_type in ["square", "sawtooth", "triangle", "pulse"]:
        return np.pi  # Period 2π
    elif wave_type == "cubic":
        return 1  # Period 2
